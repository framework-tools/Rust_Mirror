...
pub fn actualise_mark_step_on_standard_blocks(
    ...
    if !top_from_block.parent_is_root(&block_map) || mark_step.from.block_id == mark_step.to.block_id {
        ...
        if !top_from_block.parent_is_root(&block_map) {
            ...
            for block_id in parent_block.children[from_index + 1..=to_index].iter() {
                ...
                if child.id() == to_deepest_std_block.id() {
                    ...
                    child.apply_mark_to_all_inline_blocks(mark_step.mark.clone(), &mut block_map, add_mark, &mut blocks_to_update)?;
                    apply_mark_to_descendants_inline_blocks(
                        &child,
                        &mark_step.mark,
                        &mut block_map,
        ...
        apply_mark_to_all_lower_relatives(&from_block, &mark_step.mark, &mut block_map, add_mark, &mut blocks_to_update)?;
        let to_second_deepest_layer = mark_step.to.clone().get_two_deepest_layers()?;
        let to_deepest_layer = mark_step.to.get_deepest_subselection();
        ...
        if &to_block._id != &mark_step.to.block_id {
            ...
            to_block_highest.apply_mark_to_all_inline_blocks_in_range(
                mark_step.mark.clone(),
                0,
                to_block_highest.content_block()?.inline_blocks.len() - 1,
                &mut block_map,
                add_mark,
                &mut blocks_to_update
            )?;
        ...
        if from_block_index != to_block_index {
            for id in highest_parent_children[from_block_index + 1..to_block_index].iter() {
                ...
                apply_mark_to_descendants_inline_blocks(&block, &mark_step.mark, &mut block_map, None, add_mark, &mut blocks_to_update)?;
            }
        }
        ...
        match to_block_to_stop_at {
            Some(to_block_to_stop_at) => {
                if id == to_block_to_stop_at.0 {
                    ...
                    child.apply_mark_to_all_inline_blocks(mark.clone(), block_map, add_mark, blocks_to_update)?;
                    if apply_mark_to_descendants_inline_blocks(
                        &child,
                        mark,
                        block_map,
                        Some(to_block_to_stop_at),
            ...
            None => {
                child.apply_mark_to_all_inline_blocks(mark.clone(), block_map, add_mark, blocks_to_update)?;
                apply_mark_to_descendants_inline_blocks(&child, mark, block_map, None, add_mark, blocks_to_update)?;
            }
        };
    }
...
fn apply_mark_to_all_lower_relatives(
    ...
    while i < parent.children.len() {
        ...
        younger_sibling.apply_mark_to_all_inline_blocks(mark.clone(), block_map, add_mark, blocks_to_update)?;
        apply_mark_to_descendants_inline_blocks(&younger_sibling, mark, block_map, None, add_mark, blocks_to_update)?;
        i += 1;
    }

    apply_mark_to_all_lower_relatives(&parent, mark, block_map, add_mark, blocks_to_update)?;
    return Ok(())
}
