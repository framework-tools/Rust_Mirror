...
use crate::{step::{MarkStep}, blocks::{BlockMap, Block, inline_blocks::InlineBlock}, steps_generator::StepError};

use super::clean_block_after_transform;


pub fn execute_mark_step(mark_step: MarkStep, mut block_map: BlockMap, add_mark: bool) -> Result<BlockMap, StepError> {
    let block = block_map.get_block(&mark_step.from.block_id)?;
    match block {
        Block::InlineBlock(inline_block) => {
            return execute_mark_step_on_inline_block(mark_step, inline_block, block_map, add_mark)
        }
...
fn execute_mark_step_on_inline_block(
    ...
    if mark_step.from.block_id == mark_step.to.block_id {
        ...
        let mut middle_block = from_block.clone().to_new_block().update_text(middle_text)?;
        let after_block = from_block.to_new_block().update_text(after_text)?;
        ...
        content_block.inline_blocks.splice(
            ...
            vec![before_block._id, middle_block._id, after_block._id]
        );
        ...
        let before_from_block_id = before_from_block._id;
        block_map.update_block(Block::InlineBlock(before_from_block))?;
        let after_from_block = from_block.to_new_block().update_text(after_from_text)?;
        let after_from_block_id = after_from_block._id;
