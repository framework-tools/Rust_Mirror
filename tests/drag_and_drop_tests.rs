...
mod tests {
    ...
    #[test]
    fn can_drop_block_below_child_to_child_within_same_parent() {
        let mut new_ids = NewIds::hardcoded_new_ids_for_tests();
        ...
        let block_map = BlockMap::from(vec![
            ...
            paragraph_block3.to_string(), root_block.to_string()
        ]).unwrap();
        let event = Event::DropBlock(DropBlockEvent {
            drag_block_id: paragraph_block_id2.clone(),
            drop_block_id: paragraph_block_id3.clone(),
            side_dropped: Side::Bottom
        });
        // selection does not matter for this event
        let sub_selection = SubSelection::from(inline_block_id2.clone(), 4, None);
        ...
        assert_eq!(updated_paragraph_block1.children, vec![paragraph_block_id3.clone(), paragraph_block_id2.clone()]);
    }
    #[test]
    fn can_drop_block_from_root_to_a_child_within_same_parent() {
        let mut new_ids = NewIds::hardcoded_new_ids_for_tests();
        ...
        let root_block = RootBlock::json_from(root_block_id.clone(), vec![paragraph_block_id1.clone(), paragraph_block_id3.clone()]);
        let block_map = BlockMap::from(vec![
            inline_block1.to_string(), inline_block2.to_string(), paragraph_block1.to_string(), paragraph_block2.to_string(),
            paragraph_block3.to_string(), root_block.to_string()
        ]).unwrap();
