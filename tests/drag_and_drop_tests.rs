...
mod tests {
    ...
    fn can_drop_block_on_side_inside_layout_block() {
        let mut new_ids = NewIds::hardcoded_new_ids_for_tests();
        ...
        let layout_block = json!({
            ...
            "_type": "layout",
            "content": {
                "blocks": [paragraph_block_id1.clone(), paragraph_block_id2.clone()],
                "horizontal": true
            },
            "marks": [],
            "parent": root_block_id.clone()
        ...
        let root_block = RootBlock::json_from(root_block_id.clone(), vec![layout_block_id.clone(), paragraph_block_id3.clone()]);
        let block_map = BlockMap::from(vec![
            inline_block1.to_string(), inline_block2.to_string(), paragraph_block1.to_string(), paragraph_block2.to_string(), root_block.to_string(),
            layout_block.to_string()
        ]).unwrap();
        let event = Event::DropBlock(DropBlockEvent {
            drag_block_id: paragraph_block_id3.clone(),
            drop_block_id: paragraph_block_id1.clone(),
        ...
        match layout_block.content{
            StandardBlockType::Layout(layout_block) => {
                assert_eq!(layout_block.blocks, vec![paragraph_block_id1.clone(), paragraph_block_id3.clone(), paragraph_block_id2.clone()]);
                assert_eq!(layout_block.horizontal, true);
            },
            _ => panic!("Expected layout block")
        };
